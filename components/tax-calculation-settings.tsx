...
// Define tax calculation settings interface
...
// Form schema for tax calculation rate settings
const rateFormSchema = z.object({
  personalExpenseRate: z.coerce
    .number()
    .min(0, "อัตราเงินหักค่าใช้จ่ายส่วนบุคคลต้องไม่ต่ำกว่า 0%")
    .max(100, "อัตราเงินหักค่าใช้จ่ายส่วนบุคคลต้องไม่เกิน 100%"),
  maxDeductionAmount: z.coerce.number().min(0, "จำนวนเงินหักสูงสุดต้องไม่ต่ำกว่า 0 บาท"),
  personalAllowance: z.coerce.number().min(0, "ค่าลดหย่อนตนเองต้องไม่ต่ำกว่า 0 บาท"),
})
...
// Mock data for tax calculation settings
const mockTaxCalculationSettings: Record<string, TaxCalculationSettings> = {
  "1": {
    personalExpenseRate: 50,
...
export function TaxCalculationSettings({ companyId }: TaxCalculationSettingsProps) {
  const { toast } = useToast()
  const [employeeTypes, setEmployeeTypes] = useState<EmployeeType[]>([])
  const [incomeItems, setIncomeItems] = useState<IncomeItem[]>([])
  const [settings, setSettings] = useState<TaxCalculationSettings | null>(null)
  ...
  useEffect(() => {
    ...
    setSettings(mockTaxCalculationSettings[companyId] || null)

    if (mockTaxCalculationSettings[companyId]) {
      rateForm.reset({
        personalExpenseRate: mockTaxCalculationSettings[companyId].personalExpenseRate,
        maxDeductionAmount: mockTaxCalculationSettings[companyId].maxDeductionAmount,
        personalAllowance: mockTaxCalculationSettings[companyId].personalAllowance,
      })
    }
  }, [companyId, rateForm])
  ...
  const handleRateSubmit = (data: RateFormValues) => {
    ...
    const updatedSettings = {
      ...settings,
      personalExpenseRate: data.personalExpenseRate,
      maxDeductionAmount: data.maxDeductionAmount,
    ...
    toast({
      ...
      description: "อัตราการคำนวณภาษีได้รับการอัปเดตเรียบร้อยแล้ว",
    })
  }
  ...
  const handleSaveIncomeItems = () => {
    toast({
      title: "บันทึกการตั้งค่าสำเร็จ",
      description: "รายการเงินได้ที่ใช้คำนวณภาษีได้รับการอัปเดตเรียบร้อยแล้ว",
    })
  }
  ...
  return (
    <div className="space-y-8">
      {/* Section 1: Tax Calculation Rate Settings */}
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <div>
            <CardTitle>กำหนดอัตราการคำนวณภาษี</CardTitle>
            <CardDescription>กำหนดอัตราเงินหักค่าใช้จ่ายส่วนบุคคล จำนวนเงินหักสูงสุด และค่าลดหย่อนตนเอง</CardDescription>
          </div>
        ...
        <CardContent>
          {isEditing ? (
              ...
              <form onSubmit={rateForm.handleSubmit(handleRateSubmit)} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <FormField
                    ...
                    name="personalExpenseRate"
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>กำหนดอัตราเงินหักค่าใช้จ่ายส่วนบุคคล</FormLabel>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>กำหนดอัตราเงินหักสูงสุดไม่เกิน</FormLabel>
                        <div className="flex items-center space-x-2">
                          <FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>กำหนดอัตราเงินหักค่าลดหย่อนตนเอง</FormLabel>
                        <div className="flex items-center space-x-2">
                          <FormControl>
                            <Input type="number" step="1" min="0" {...field} />
                          </FormControl>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <h3 className="text-sm font-medium text-muted-foreground mb-2">กำหนดอัตราเงินหักค่าใช้จ่ายส่วนบุคคล</h3>
                <p className="text-xl font-semibold">{settings.personalExpenseRate}%</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-muted-foreground mb-2">กำหนดอัตราเงินหักสูงสุดไม่เกิน</h3>
                <p className="text-xl font-semibold">{settings.maxDeductionAmount.toLocaleString()} บาท</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-muted-foreground mb-2">กำหนดอัตราเงินหักค่าลดหย่อนตนเอง</h3>
                <p className="text-xl font-semibold">{settings.personalAllowance.toLocaleString()} บาท</p>
              </div>
            </div>
          )}
        </CardContent>
      ...
      {/* Section 2: Income Items for Tax Calculation */}
      <Card>
        <CardHeader>
          <CardTitle>รายการเงินได้เพื่อคำนวณภาษีหัก ณ ที่จ่าย</CardTitle>
          <CardDescription>เลือกรายการเงินได้ที่ต้องการนำมาคำนวณภาษีหัก ณ ที่จ่ายตามประเภทพนักงาน</CardDescription>
        </CardHeader>
        <CardContent>
